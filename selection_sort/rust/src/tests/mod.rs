...
fn test_selection_sort_edgecases(to_sort_vector: Vec<i32>, expected_sorted_vector: Vec<i32>) {
    assert_eq!(expected_sorted_vector, selection_sort(to_sort_vector));
}

proptest! {
    #[test]
    fn test_selection_sort(to_sort_vector in prop::collection::vec(-1000000i32..1000000, 1..1000)) {
        //Given
        let expected_length = to_sort_vector.len();

        //When
        let returned_sorted_vector = selection_sort(to_sort_vector);
